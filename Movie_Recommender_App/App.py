...
def movie_poster_fetcher(imdb_link):
    url_data = requests.get(imdb_link, headers=hdr).text
    s_data = BeautifulSoup(url_data, 'html.parser')
    imdb_dp = s_data.find("meta", property="og:image")  # Update property to fetch the poster link
    ...
    if imdb_dp is not None and 'content' in imdb_dp.attrs:
        ...
        try:
            ...
            st.warning("Failed to fetch the movie poster.")
    else:
        st.warning("Movie poster link not found for this movie.")
...
def get_movie_info(imdb_link):
    ...
    if imdb_content is not None and 'content' in imdb_content.attrs:
        ...
        if len(movie_descr) >= 3:
            ...
            movie_rating = 'Total Rating count: ' + str(rating.text) if rating else ''

            return movie_director, movie_cast, movie_story, movie_rating
        else:
            return "Data structure mismatch", "", "", ""
    else:
        return "No movie description found", "", "", ""
